...
export const WorkspaceHero = ({
  ...
  metrics,
  actions,
  accent = 'from-[#1f3d74] via-[#17254a] to-[#170f2f]',
  footerContent = null,
}) => (
  <section className={`newgears-workspace-hero newgears-hud-shell relative overflow-hidden border border-white/12 bg-gradient-to-br ${accent} p-6 md:p-8 shadow-[0_36px_110px_rgba(0,0,0,0.34)]`}>
    ...
    <div className="pointer-events-none absolute left-6 top-5 h-24 w-24 border-l-2 border-t-2 border-cyan-300/25" />
    <div className="pointer-events-none absolute bottom-6 right-6 h-24 w-24 border-b-2 border-r-2 border-yellow-300/20" />
    <div className="pointer-events-none absolute -left-12 top-0 h-44 w-44 rounded-full bg-[#1ef2ff]/18 blur-3xl" />
    <div className="pointer-events-none absolute right-[-38px] top-[-14px] h-48 w-48 rounded-full bg-[#ffe76a]/16 blur-3xl" />
    <div className="pointer-events-none absolute bottom-[-84px] right-[8%] h-64 w-64 rounded-full bg-[#6cff8d]/12 blur-3xl" />
    ...
    <div className="relative z-10 grid gap-7 xl:grid-cols-[1.08fr,0.92fr]">
      <div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="newgears-status-strip inline-flex items-center gap-2 border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.24em] text-cyan-100">
            <Radio size={12} />
            Interface ativa
          </span>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <div className="newgears-status-strip inline-flex items-center gap-2 border border-lime-400/20 bg-lime-400/10 px-4 py-3 text-[10px] font-black uppercase tracking-[0.22em] text-lime-100">
            <Shield size={12} />
            Leitura pronta
          </div>
        </div>
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          {actions.map((action) => (
            <button
              ...
              className={`newgears-workspace-action inline-flex items-center gap-2 border px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-white shadow-[0_18px_36px_rgba(0,0,0,0.2)] transition-all hover:-translate-y-1 ${action.style}`}
            >
              {action.icon}
              {action.label}
            </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        {metrics.map((metric, index) => (
          <div key={metric.label} className="newgears-workspace-metric border border-white/12 bg-black/24 p-4 shadow-[0_20px_40px_rgba(0,0,0,0.18)] backdrop-blur-sm">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className={`border px-2.5 py-1.5 shadow-[0_12px_24px_rgba(0,0,0,0.12)] ${metric.tone || 'border-white/10 bg-white/10 text-white'}`}>
                {metric.icon}
              </div>
            </div>
            ...
            <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/8">
              <div
                className="newgears-scan-bar h-full rounded-full bg-gradient-to-r from-cyan-300 via-blue-400 to-lime-300"
    ...
    <div className="newgears-tabs-shell newgears-hud-shell sticky top-[90px] z-30 overflow-hidden border border-white/12 bg-[linear-gradient(145deg,rgba(15,18,35,0.95),rgba(11,16,31,0.94))] shadow-[0_30px_80px_rgba(0,0,0,0.32)]">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="max-w-3xl">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <span className="newgears-status-strip inline-flex items-center gap-2 border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100">
                <ScanSearch size={12} />
                Aba ativa
              </span>
      ...
      <div className="flex gap-2 overflow-x-auto p-2.5 custom-scrollbar">
        {tabs.map((tab, index) => (
          ...
          >
            <span className={`absolute left-0 top-0 h-full w-1.5 ${activeId === tab.id ? 'bg-gradient-to-b from-cyan-300 via-blue-400 to-lime-300' : 'bg-white/8'}`} />
            <span className="inline-flex h-10 min-w-10 items-center justify-center rounded-[14px] border border-white/12 bg-black/25 text-[11px] font-black text-white/85">
            ...
            {tab.badge ? (
              <span className="ml-auto inline-flex h-[22px] min-w-[22px] items-center justify-center rounded-full bg-black/24 px-1.5 text-[10px] font-black">
                {tab.badge}
              </span>
            ) : null}
          </button>
...
export const WorkspacePanelToolbar = ({
  ...
  return (
    <div className="newgears-toolbar-shell newgears-hud-shell border border-white/12 bg-[linear-gradient(145deg,rgba(15,18,35,0.95),rgba(11,16,31,0.94))] p-4 shadow-[0_28px_76px_rgba(0,0,0,0.28)]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            <span className="newgears-status-strip inline-flex items-center gap-2 border border-lime-400/20 bg-lime-400/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.22em] text-lime-100">
              <Shield size={12} />
              HUD pronto
            </span>
          </div>
      ...
      <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white/8">
        <div
          className="newgears-scan-bar h-full rounded-full bg-gradient-to-r from-lime-300 via-cyan-300 to-blue-400"
