...
const RobotDesignStrategyPanel = ({
  ...
  const [showRobotGuide, setShowRobotGuide] = React.useState(false);
  ...
  const pendingCodeCount = sortedCodeSnippets.filter((item) => !item.applied).length;
  ...
  const focusCards = [
    {
      label: 'Cofre de codigos',
      value: sortedCodeSnippets.length,
      helper: activeCommandCode ? `Oficial: ${activeCommandCode.title}` : 'Definam a base oficial da equipe.',
      tone: 'border-green-500/20 bg-green-500/10 text-green-200',
    },
    {
      label: 'Diario de garras',
      value: sortedAttachments.length,
      helper: latestAttachment ? `Ultima: ${latestAttachment.name}` : 'Registrem os anexos principais.',
      tone: 'border-cyan-500/20 bg-cyan-500/10 text-cyan-200',
    },
    {
      label: 'Diario do robo',
      value: sortedRobotVersions.length,
      helper: latestRobotVersion ? `Ultima: ${latestRobotVersion.version}` : 'Documentem a evolucao do robo.',
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <section className="newgears-major-panel relative overflow-hidden rounded-[30px] border border-white/10 bg-gradient-to-br from-[#111b2f] via-[#101726] to-[#0e111b] p-6 md:p-8 shadow-[0_25px_80px_rgba(0,0,0,0.32)]">
        ...
        <div className="relative z-10 grid gap-6 xl:grid-cols-[0.94fr,1.06fr]">
          <div>
            ...
            <h3 className="mt-4 text-3xl font-black leading-tight text-white">Foco no que a equipe mais usa</h3>
            <p className="mt-4 max-w-3xl text-sm leading-relaxed text-gray-300">
              A tela principal agora destaca o que realmente ajuda no dia a dia do time: cofre de codigos, diario de garras e diario do robo. O guia mais completo da banca ficou separado para nao poluir a area principal.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <button onClick={() => setShowRobotGuide((current) => !current)} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-bold text-white transition-all hover:bg-white hover:text-black">
                <Sparkles size={14} /> {showRobotGuide ? 'Ocultar guia' : 'Ver guia do robo'}
              </button>
            </div>
          </div>
          <div className="rounded-[28px] border border-white/10 bg-black/25 p-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-gray-500 font-bold">Principal</p>
            ...
            <div className="mt-4 rounded-[22px] border border-white/10 bg-white/5 p-4">
              <p className="text-[10px] uppercase tracking-[0.18em] text-gray-500 font-bold">Resumo separado</p>
              <p className="mt-3 text-xs leading-relaxed text-gray-300">
                Garage do Robo, leitura da banca e guia de defesa agora ficam em um painel separado. Assim o time bate o olho no que usa toda hora e abre o guia so quando precisar revisar a apresentacao.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid xl:grid-cols-[1.08fr,0.92fr] gap-6">
          <div className="bg-[#151520] border border-white/10 rounded-[28px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-gray-500 font-bold">Garage do Robo</p>
                <h3 className="text-xl font-black text-white mt-2">O que a banca enxerga</h3>
              </div>
              <span className={`rounded-full border px-3 py-1 text-[10px] uppercase tracking-[0.18em] font-bold ${readinessTone}`}>
          ...
          <div className="bg-[#151520] border border-white/10 rounded-[28px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <p className="text-[10px] uppercase tracking-[0.22em] text-gray-500 font-bold">Guia de apresentacao</p>
                <h3 className="text-xl font-black text-white mt-2">Como deixar a banca curiosa</h3>
              </div>
              <button onClick={() => setShowRobotGuide(false)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] uppercase tracking-[0.18em] font-bold text-gray-200 transition-all hover:bg-white/10 hover:text-white">Ocultar guia</button>
            </div>
      ...
      <div className="grid xl:grid-cols-[0.95fr,0.95fr,1.1fr] gap-6">
        <div className="order-3 bg-[#151520] border border-white/10 rounded-[28px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
          <div className="flex justify-between items-center mb-6"><div><p className="text-[10px] uppercase tracking-[0.22em] text-gray-500 font-bold">Evolucao do robo</p><h3 className="text-lg font-black text-white flex items-center gap-2 mt-2"><GitCommit className="text-blue-500" /> Diario do Robo</h3></div><button onClick={onOpenRobot} className="text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 px-3 py-1.5 rounded-lg hover:bg-blue-500 hover:text-white font-bold transition-all">+ Versao</button></div>
        ...
        <div className="order-2 bg-[#151520] border border-white/10 rounded-[28px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.22)]">
          <div className="flex justify-between items-center mb-6"><div><p className="text-[10px] uppercase tracking-[0.22em] text-gray-500 font-bold">Garras e acessorios</p><h3 className="text-lg font-black text-white flex items-center gap-2 mt-2"><Wrench className="text-cyan-500" /> Diario de Garras</h3></div><button onClick={onOpenAttachment} className="text-xs bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 px-3 py-1.5 rounded-lg hover:bg-cyan-500 hover:text-black font-bold transition-all">+ Garra</button></div>
          ...
          <div className="space-y-4">
            {sortedCodeSnippets.map((code) => (
              <div key={code.id} onClick={() => onOpenCodeView(code)} className={`border p-4 rounded-xl relative transition-colors group cursor-pointer ${code.applied ? 'bg-green-500/10 border-green-500/30 hover:bg-green-500/15' : 'bg-black/40 border-white/5 hover:bg-white/5'}`}>
                ...
                {isAdmin && !code.applied ? (
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <button
                      onClick={(event) => { event.stopPropagation(); onApplyCode(code); }}
                      className="inline-flex items-center gap-2 rounded-xl border border-green-500/20 bg-green-500/10 px-3 py-2 text-xs font-bold text-green-200 transition-all hover:bg-green-500 hover:text-black"
                    >
                      <Laptop size={14} />
